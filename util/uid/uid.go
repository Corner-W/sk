...
import (
	"errors"
	"github.com/Corner-W/sk/util/hash"
	"github.com/Corner-W/sk/util/rand"
	"net"
	"os"
	"strconv"
	"time"
...
func init() {
	...
	if len(macs) > 0 {
		...
		s := strconv.FormatUint(uint64(hash.SDBM(macAddrBytes)), 36)
		copy(b, s)
		macAddrStr = string(b)
	}
...
func GetMacArray() (macs []string, err error) {
	...
	if err != nil {
		err = errors.New(`net.Interfaces failed`)
		return nil, err
	}
...
func getSequence() []byte {
	b := []byte{'0', '0', '0'}
	s := strconv.FormatUint(uint64((sequence+1)%sequenceMax), 36)
	copy(b, s)
	return b
}
...
func getDataHashStr(data []byte) []byte {
	b := []byte{'0', '0', '0', '0', '0', '0', '0'}
	s := strconv.FormatUint(uint64(hash.SDBM(data)), 36)
	copy(b, s)
	return b
}
