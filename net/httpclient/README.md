...
```bash
# Go Modules
require github.com/go-httpclient/httpclient/v2 v2.7.0
```
...
import "github.com/go-httpclient/httpclient/v2"
```
...
Response Info:
  ...
  Body       :
  {
    ...
    "headers": {
      ...
      "Host": "httpbin.org",
      "User-Agent": "go-httpclient/2.4.0 (https://github.com/go-resty/resty)",
...
resp, err := client.R().
      SetQueryString("productId=232&template=fresh-sample&cat=httpclient&source=google&kw=buy a lot more").
      SetHeader("Accept", "application/json").
      SetAuthToken("BC594900518B4F7EAC75BD37F019E08FBC594900518B4F7EAC75BD37F019E08F").
      Get("/show_product")
...
// See we are not setting content-type header, since go-httpclient automatically detects Content-Type for you
resp, err := client.R().
      SetBody(fileBytes).
      SetContentLength(true).          // Dropbox expects this value
      SetAuthToken("<your-auth-token>").
      SetError(&DropboxError{}).       // or SetError(DropboxError{}).
      Post("https://content.dropboxapi.com/1/files_put/auto/httpclient/mydocument.pdf") // for upload Dropbox supports PUT too

// Note: httpclient detects Content-Type for request body/payload if content type header is not set.
...
resp, err := client.R().
      SetBody(Article{
        ...
        Tags: []string{"article", "sample", "httpclient"},
      }).
...
// Setting output directory path, If directory not exists then httpclient creates one!
...
// Using raw func into httpclient.SetRedirectPolicy
...
client.R().SetQueryString("productId=232&template=fresh-sample&cat=httpclient&source=google&kw=buy a lot more")
...
client.SetError(&Error{})    // or httpclient.SetError(Error{})
```
...
// Create a Go's http.Transport so we can set it in httpclient.
