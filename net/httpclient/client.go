...
// httpclient source code and usage is governed by a MIT style
...
var (
	...
	hdrUserAgentValue = "go-httpclient/" + Version + " (https://github.com/go-httpclient/httpclient)"
...
// SetCookieJar method sets custom http.CookieJar in the httpclient client. Its way to override default.
//
// For Example: sometimes we don't want to save cookies in api contacting, we can remove the default
// CookieJar in httpclient client.
//
...
//	cookies := []*http.Cookie{
//		&http.Cookie{
//			Name:"go-httpclient-1",
//			Value:"This is cookie 1 value",
//		},
//		&http.Cookie{
//			Name:"go-httpclient-2",
//			Value:"This is cookie 2 value",
//		},
//	}
//
//	// Setting a cookies into httpclient
...
//
// For Example: To set the header for username "go-httpclient" and password "welcome"
//
//	client.SetBasicAuth("go-httpclient", "welcome")
//
...
//
//	client.OnBeforeRequest(func(c *httpclient.Client, r *httpclient.Request) error {
...
//
//	client.OnAfterResponse(func(c *httpclient.Client, r *httpclient.Response) error {
...
//
//	client.OnError(func(req *httpclient.Request, err error) {
//		if v, ok := err.(*httpclient.ResponseError); ok {
...
// SetPreRequestHook method sets the given pre-request function into httpclient client.
...
// called before the httpclient actually logs the information.
...
//
// Compliant to interface `httpclient.Logger`.
...
func (c *Client) SetRedirectPolicy(policies ...interface{}) *Client {
	for _, p := range policies {
		if _, ok := p.(RedirectPolicy); !ok {
			c.log.Errorf("%v does not implement httpclient.RedirectPolicy (missing Apply method)",
				functionName(p))
		}
	}
...
// If the output directory not exists then httpclient creates one. This setting is optional one,
...
// compatible interface implementation in the httpclient client.
//
// Note:
//
...
//
//	client := httpclient.New().EnableTrace()
//
...
// IsProxySet method returns the true is proxy is set from httpclient client otherwise
// false. By default proxy is set from environment, refer to `http.ProxyFromEnvironment`.
func (c *Client) IsProxySet() bool {
	return c.proxyURL != nil
}

// GetClient method returns the current `http.Client` used by the httpclient client.
...
func (c *Client) execute(req *Request) (*Response, error) {
	...
	// to modify the *httpclient.Request object
